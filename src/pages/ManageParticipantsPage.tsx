...
const ManageParticipantsPage: React.FC = () => {
  ...
  const participants = useMemo(() => getStudentList("participants"), [getStudentList]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    const result = await upsertStudent(editingStudent as Omit<Student, "id">, "participants");
    setIsSaving(false);
  ...
  const handleDelete = async (rollNo: string) => {
    if (!confirm(`Are you sure you want to delete participant with Roll No: ${rollNo}?`)) return;

    const result = await deleteStudent(rollNo, "participants");
    if (result.success) {
      toast.success(result.message);
    } else {
      toast.error(result.message);
    ...
    <div className="px-3 sm:px-6 py-6 space-y-6 max-w-screen-xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-primary font-display">Manage Participants</h1>
          <p className="text-sm text-muted-foreground">Add, edit, or remove participants ({participants.length} total)</p>
        </div>
        <button
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search by name, roll number, or team..."
          value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredParticipants.map((p) => (
            <div key={p.rollNo} className="p-4 rounded-xl border bg-card hover:shadow-md transition-shadow space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-0.5 min-w-0">
                  <p className="font-bold text-card-foreground leading-tight truncate">{p.name}</p>
                  <p className="text-xs text-muted-foreground font-mono">{p.rollNo}</p>
                </div>
                {p.teamName && (
                  <span className="shrink-0 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold whitespace-nowrap">
                    {p.teamName}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="space-y-0.5">
                  ...
                  <p className="text-card-foreground">{p.phoneNumber}</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-muted-foreground font-medium">Department</p>
                  <p className="text-card-foreground">{p.department} • Year {p.year}</p>
                </div>
              </div>
          ...
          <div className="w-full max-w-md bg-card rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h2 className="text-lg font-bold font-display">
                {editingStudent?.rollNo ? "Edit Participant" : "Add New Participant"}
              </h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingStudent({...editingStudent!, teamName: e.target.value.toUpperCase()})}
                    className="w-full px-3 py-2 rounded-lg border bg-muted/30 focus:outline-none focus:ring-2 focus:ring-primary/20 text-sm font-medium"
